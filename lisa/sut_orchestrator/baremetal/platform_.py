...
from .cluster.cluster import Cluster, ClusterCapabilities
...
class BareMetalPlatform(Platform):
    ...
    def _prepare_environment(self, environment: Environment, log: Logger) -> bool:
        ...
        self.cluster_instance = self._baremetal_runbook.cluster[0]

        self.cluster = self.cluster_factory.create_by_runbook(self.cluster_instance)
        assert self.cluster.runbook.client, "no client is specified in the runbook"
        return self._configure_node_capabilities(
            environment, log, self.cluster.get_cluster_capabilities()
        )
    ...
    def _deploy_environment(self, environment: Environment, log: Logger) -> None:
        ...
        if self.cluster_instance.ready_checker:
            ready_checker = self.ready_checker_factory.create_by_runbook(
                self.cluster_instance.ready_checker
            )
        ...
        for index, node in enumerate(environment.nodes.list()):
            ...
            # get ip address
            if self.cluster_instance.ip_getter:
                ip_getter = self.ip_getter_factory.create_by_runbook(
                    self.cluster_instance.ip_getter
                )
    ...
    def _create_node_capabilities(
        self, cluster_capabilities: ClusterCapabilities
    ) -> schema.NodeSpace:
